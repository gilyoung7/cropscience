#!/usr/bin/env bash
#
# Stage 2 split3-only batch with PER-PEST best Stage 1 gate.
#
# Differs from run_stage2_split3_2024_best_gate_batch.sh by ACTUALLY USING
# the selected (method, run) as the Stage 2 alert source:
#   selected_method == A_baseline         -> A_raw_global gate (single tau, k)
#   selected_method == D_history          -> D_raw_global gate (single tau, k)
#   selected_method == dispatch_group_tau -> with_history split (current default)
#
# All 14 dispatch feature columns (+ missing indicator = 15 chans) are still
# computed; under A_baseline / D_history the dispatch_branch column collapses
# to a constant value, and dispatch_score / tau_used / streak / slope come
# from the gate's own series — so Stage 2 input d_in=45 is preserved.
#
# split3: val=2023 / test=2024. 8 pests, BPH2 excluded.
...
BATCH_EVAL_OVERRIDE=""
# Gaussian loss-mode ablation knobs (Phase: interval NLL). Default values
# preserve the existing asym_mse pipeline so other runs are not affected.
# Override via env: GAUSSIAN_LOSS_MODE=interval_nll GAUSSIAN_INTERVAL_CC=0
GAUSSIAN_LOSS_MODE="${GAUSSIAN_LOSS_MODE:-asym_mse}"
GAUSSIAN_INTERVAL_CC="${GAUSSIAN_INTERVAL_CC:-0}"
# Mixed-loss lambda (asym_mse + λ * interval_nll). Empty = phase_s5 default 0.1.
GAUSSIAN_INTERVAL_LAMBDA="${GAUSSIAN_INTERVAL_LAMBDA:-}"
# Event-only fine-tuning hooks. Defaults empty => no change to legacy pipeline.
#   SKIP_PILOT_WARM_FROM=<dir>  : skip phase_s5 pilot stage; warm-start final
#                                 stage from <dir>/ckpt/checkpoint_runN.pt
#                                 (e.g. existing asym_mse lead_v3_final ckpt).
#   RIGHT_WEIGHT=<float>        : override --stage2_pmf_right_weight (0=event
#                                 only). Empty = keep phase_s5 default 0.3.
#   MAX_EPOCHS=<int>            : cap training epochs (forwarded as
#                                 --max_epochs_override). Empty = full schedule.
SKIP_PILOT_WARM_FROM="${SKIP_PILOT_WARM_FROM:-}"
RIGHT_WEIGHT="${RIGHT_WEIGHT:-}"
MAX_EPOCHS="${MAX_EPOCHS:-}"
# Residual-from-climatology mu head ablation. Defaults preserve legacy
# lead_from_alert mu head used by the bestgate baseline.
#   MU_MODE=residual_clim  : enable residual head. Wrapper auto-looks up
#                            clim_mid from existing climatology_train_stats.csv
#                            (siblings of SKIP_PILOT_WARM_FROM, or the new
#                            ${PEST_OUT}/climatology_train_stats.csv if cold).
#   DELTA_MAX=<float>      : tanh-bounded delta half-range (days). Default 60.
#   RESET_HEAD_MU_FINAL=1  : drop head_mu weights from the final-stage warm
#                            start so delta starts at 0 (mu = clim_mid).
MU_MODE="${MU_MODE:-}"
DELTA_MAX="${DELTA_MAX:-60}"
RESET_HEAD_MU_FINAL="${RESET_HEAD_MU_FINAL:-0}"
# Loss-target overrides (forwarded to phase_s5_train.py). Empty = legacy.
TARGET_MODE="${TARGET_MODE:-}"
ASYM_WEIGHT_EARLY="${ASYM_WEIGHT_EARLY:-}"
# Reset-only sanity bypass. When set with MU_MODE=residual_clim:
#   - skip phase_s5_train entirely (no training)
#   - zero-init head_mu.* tensors in the warm-start ckpt
#   - overwrite ckpt metadata to declare residual_clim + clim_mid + delta_max
#   - save as the final ckpt at the new out_root
#   - continue with sample_grid / climatology / canonical_summary normally
# If the residual_clim wiring is correct, sample_grid IoU should match the
# climatology_mean_mid baseline (since delta=0 forces mu = clim_mid for all).
RESET_ONLY="${RESET_ONLY:-0}"
OUT_ROOT="rice/outputs_stage2_batch_2024_bestgate"
SUMMARY_ROOT="${OUT_ROOT}/_summary"
GATE_CSV="${SUMMARY_ROOT}/stage1_gate_selection_split3_2024.csv"
FAIL_LOG=""

# uncond candidates (per pest). Train one if missing.
...
usage() {
  cat <<EOF
Usage: $0 [--pests "p1 p2 ..."] [--force] [--low-mem]
  ...
  --batch_train N      explicit override (passed to run_train + phase_s5_train)
...
# Resolve low-mem: explicit overrides win; otherwise --low-mem implies 16/32.
if [[ "$LOW_MEM" == "1" ]]; then
...
echo "================================================================"
echo "Stage 2 split3-only PEST-BEST-GATE batch (val=$VAL_YEAR test=$TEST_YEAR_MIN..$TEST_YEAR_MAX)"
echo "  pests: $PESTS"
...
for PEST in $PESTS; do
  ...
  echo "  gate: method=$SEL_METHOD  run=$SEL_RUN  (used as Stage 2 alert source)"
  echo "  A_ckpt   = $A_CKPT"
  echo "  D_ckpt   = $D_CKPT"
  ...
  # NOTE: dispatch CSV name encodes the gate method so different gates do not
  # share a stale CSV when the same out_root is reused.
  ...
  # Event-only fine-tuning shortcut: if SKIP_PILOT_WARM_FROM is set, skip the
  # pilot stage and warm-start the final stage from the given ckpt directory.
  # The user-supplied path replaces $PILOT_OUT as the source of pilot_ckpt;
  # _check_ckpt in phase_s5_train will verify it exists.
  ...
  if [[ "$MU_MODE" == "residual_clim" ]]; then
    # Look up clim_mid from an existing climatology_train_stats.csv. Prefer the
    # sibling of the warm-start ckpt (re-uses the bestgate baseline's stats
    # without recomputing); else look under the new PEST_OUT (cold start case).
    CLIM_CSV=""
    if [[ -n "$SKIP_PILOT_WARM_FROM" ]]; then
  ...
  if [[ "$RESET_ONLY" == "1" ]]; then
    # Sanity-check bypass: skip training entirely. Zero-init head_mu in the
    # warm-start ckpt, declare residual_clim metadata, save to FINAL_CKPT.
    # Downstream sample_grid + canonical_summary then evaluate that ckpt,
    # so a working residual_clim wiring produces mu ≡ clim_mid for all
    # samples (= climatology_mean_mid baseline IoU).
...
# Force inference path to use residual_clim with the supplied clim_mid/delta_max
...
print(f"[RESET_ONLY] saved zeroed ckpt to $FINAL_CKPT  "
      f"(stage2_pmf_mu_mode=residual_clim, clim_mid=$CLIM_MID, delta_max=$DELTA_MAX)")
PYEOF
  ...
  # 4) val + test sample_grid — phase_r uses ckpt-stored dispatch CSV path,
  # but we also need to point its build_dispatch_alert_map to the SAME gate
  # method. phase_r currently reads gate_method from CLI, so we pass it via env
  # the wrapper forwards through the args.
...
echo
echo "================================================================"
echo "Aggregating all-pest selection + winner (with gate metadata)"
echo "================================================================"
...
# Count gate alerts from dispatch CSV (rows are alerted sy with their split tag).
def _alert_counts(pest, sel_method):
    csv = f"{OUT_ROOT}/{pest}/gate_{sel_method}_R088_features_per_sy.csv"
...
for pest, sub in out.groupby("pest"):
    ...
    n_val, n_test = _alert_counts(pest, sel_method)
    ...
    winners.append({
        ...
        "gate_alert_count_val":  n_val,
        "gate_alert_count_test": n_test,
...
with pd.option_context("display.width", 240, "display.max_columns", 30,
    ...
    cols = ["pest", "selected_method", "selected_run",
            "val_recall", "val_FAR", "test_recall", "test_FAR",
            "gate_alert_count_val", "gate_alert_count_test",
            "stage2_lead_v3_score", "best_climatology_model", "best_climatology_score",
            "lead_v3_beats_climatology"]
