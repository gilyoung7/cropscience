"""Phase S10 inference — sample_grid with baseline + center_ra{X} 2-sided ckpt.

Calls `phase_r_oracle_iou` with two models:
    [A] D=15 2-sided (asym=25)               (original baseline)
    [B] D=15 2-sided center_ra{X}            (this experiment)

Both use σ=5.0.  Offsets = {60, 90, 105, 120}.

Outputs:
    outputs_phase_s10_sample_grid.csv
    outputs_phase_s10_oracle.csv
"""
...
def main():
    ...
    ap.add_argument("--ckpt", type=str, required=True,
                    help="Path to the new center_ra{X} ckpt (Phase S10 final).")
    ap.add_argument("--label", type=str, default="D=15 2-sided center_ra220",
    ...
    ap.add_argument("--out_csv", type=str, default="rice/outputs/diag/phase_s10_oracle.csv")
    args = ap.parse_args()
    ...
    print("=" * 70, flush=True)
    print("Phase S10 inference (phase_r_oracle_iou)", flush=True)
