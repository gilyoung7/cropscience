"""Phase S10 — center mode + right-cens anchor shift.

Hypothesis being tested:
    Phase S8 (center mode) showed mu ≈ mid + 39, much later than the (L+R)/2
    target.  The right-censored MSE term anchors at Tend (≈ 300) by default:
        loss_right = right_weight * mean((mu − Tend)^2)
    Right-cens samples share the head_mu parameters with interval samples,
    so a Tend-anchored pull biases the head's overall output late.  S9
    confirmed this is a factor (right_weight=0 helped).  S10 keeps the
    right-cens learning signal but redirects the anchor to a less-extreme
    target (defaults to mid_max ≈ 220), so:
        loss_right = right_weight * mean((mu − right_anchor)^2)

Single-knob change vs. Phase S8:
    right_weight   : 0.3        (kept; loss is still active)
    right_anchor   : Tend (300) → 220 (CLI --right_anchor)

Everything else (target_mode='center', zone_*=0, σ=5, 3-stage chain) is fixed.

Output ckpts (default naming, "ra" = right_anchor):
    pilot:  rice/outputs_stage2_..._2sided_center_ra{value}_pilot/ckpt/checkpoint_run4.pt
    final:  rice/outputs_stage2_..._2sided_center_ra{value}/ckpt/checkpoint_run4.pt
"""
...
def _slug(x: float) -> str:
    """Filesystem-safe float slug: 220 → '220', 240 → '240', 220.5 → '220p5'."""
...
def main():
    ...
    ap.add_argument("--right_anchor", type=float, default=220.0,
                    help="DOY anchor for the right-cens MSE pull. "
                         "Common choices: 220 (≈ mid_max), 240 (≈ L_max), "
                         "300 (= Tend, legacy). 0 falls back to Tend.")
    ap.add_argument("--right_weight", type=float, default=0.3,
                    help="Right-cens MSE weight (kept active so right-cens "
                         "samples retain learning signal).")
    ...
    ra_tag = f"ra{_slug(args.right_anchor)}"
    tag = f"{base_tag}_{ra_tag}"
    ...
    print("=" * 70, flush=True)
    print(f"Phase S10 train  seed={args.seed}  target_mode={args.target_mode}  "
          f"right_weight={args.right_weight}  right_anchor={args.right_anchor}", flush=True)
    ...
    common = [
        ...
        "--stage2_pmf_right_anchor", str(args.right_anchor),
    ...
    if not args.skip_pilot:
        ...
        _run_train(f"Stage B (pilot, center, right_anchor={args.right_anchor})",
                    pilot_cmd, log_dir / f"phase_s10_train_pilot_{tag}.log")
    ...
    if not args.skip_final:
        ...
        _run_train(f"Stage C (final, center, right_anchor={args.right_anchor})",
                    final_cmd, log_dir / f"phase_s10_train_final_{tag}.log")
        _check_ckpt(final_ckpt, "final")
    else:
        print("[skip_final] stage C skipped", flush=True)

    print("\n" + "=" * 70, flush=True)
    print(f"Phase S10 train DONE  (tag={tag})", flush=True)
