"""Phase S8 inference — sample_grid with baseline + center-mode 2-sided ckpt.

Calls `phase_r_oracle_iou` with two models held side-by-side:
    [A] D=15 2-sided (asym=25)               (original baseline, l_offset mode)
    [B] D=15 2-sided center                  (this experiment, target_mode='center')

Both use σ=5.0.  Offsets = {60, 90, 105, 120} (Phase S3 C_old setup).

Outputs:
    outputs_phase_s8_sample_grid.csv
    outputs_phase_s8_oracle.csv
"""
...
def main():
    ...
    ap.add_argument("--baseline_2sided_ckpt", type=str,
                    ...
                    help="Original 2-sided ckpt (l_offset, target=L) for the comparison row.")
    ap.add_argument("--center_ckpt", type=str, required=True,
                    help="Path to the new center-mode ckpt (Phase S8 final).")
    ap.add_argument("--center_label", type=str, default="D=15 2-sided center",
                    help="Label used in the sample_grid 'model' column for the center model.")
    ap.add_argument("--sigma_2sided", type=float, default=5.0)
    ap.add_argument("--sigma_center", type=float, default=5.0)
    ...
    ap.add_argument("--out_csv", type=str, default="rice/outputs/diag/phase_s8_oracle.csv")
    args = ap.parse_args()

    for label, p in [("stage1", args.stage1_ckpt),
                      ("baseline_2sided", args.baseline_2sided_ckpt),
                      ("center", args.center_ckpt)]:
    ...
    models = (f"{base_label}|{args.baseline_2sided_ckpt};"
              f"{args.center_label}|{args.center_ckpt}")
    per_model_sigma = (f"{base_label}={args.sigma_2sided},"
                       f"{args.center_label}={args.sigma_center}")
    ...
    cmd = [
        ...
        "--oracle_sigma", str(args.sigma_center),
    ...
    print("=" * 70, flush=True)
    print("Phase S8 inference (phase_r_oracle_iou)", flush=True)
