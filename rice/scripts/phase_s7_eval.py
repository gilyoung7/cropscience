"""Phase S7 evaluation — selector (C_old) + lead-bin + shift sweep + mu sanity.

For each 2-sided variant present in the sample_grid (original baseline +
the new ε-target asym-weakened variant), this script:

    0. Prints a post-train mu_mean sanity table per model
       (L, R, mid, mu_off60/90/105/120, mu - L bias vs expected ε).
...
Outputs (out_dir, default outputs/phase_s7/):
...
Success criteria (printed at end, per Phase S7 spec):
    - mu_mean ≈ L + ε (asym weakening lets mu reach target)
    - sweet-spot (46-60) IoU loss < 0.15 (vs the −0.28 measured in S6)
    - short-lead (15-30, 31-45) IoU gain preserved (+0.30+)
    - overall IoU > 0.32
"""
...
def evaluate_one_model(label: str, grid_model: pd.DataFrame,
    ...
    print(f"  [mu sanity] L_mean={mu_row['L_mean']:.2f}  "
          ...
          f"vs expected ε={target_offset_expected}", flush=True)
...
def main():
    ...
    ap.add_argument("--sample_grid", type=str,
                    default="rice/outputs/diag/phase_s7_sample_grid.csv")
    ap.add_argument("--out_dir", type=str, default="rice/outputs/phase_s7/")
    ...
    ap.add_argument("--tc_label_substr", type=str, default="tc7p5_asym15ae0",
                    help="Substring identifying the experiment row in the grid.")
    ...
    print("\n=================== mu sanity (post-train) ===================", flush=True)
    ...
    print("\n=================== Phase S7 selector summary (C_old setup) ===================", flush=True)
    ...
    # --- Success criteria check (Phase S7 spec) ------------------------------
    if len(results) == 2:
        print("\n=================== Success criteria (Phase S7 spec) ===================", flush=True)
        ...
        # 1) overall IoU > 0.32
        ok_iou = "OK " if tc_r["iou_overall"] > 0.32 else "FAIL"
        print(f"  [{ok_iou}] overall IoU base={base_r['iou_overall']:.4f} → "
              f"tc={tc_r['iou_overall']:.4f}  (thr > 0.32)", flush=True)

        # 2) sweet-spot (46-60) loss < 0.15
        sweet_base = _bin_iou(base_r["lead_bin_decomp"], "46-60")
        sweet_tc = _bin_iou(tc_r["lead_bin_decomp"], "46-60")
        sweet_drop = sweet_base - sweet_tc
        ok_sweet = "OK " if sweet_drop < 0.15 else "FAIL"
        print(f"  [{ok_sweet}] bin 46-60 IoU base={sweet_base:.4f} → tc={sweet_tc:.4f}  "
              f"(loss = {sweet_drop:+.4f}; thr < 0.15)", flush=True)

        # 3) short-lead (15-30 + 31-45) gain preserved (sum delta ≥ +0.30)
        b_1530_d = _bin_iou(tc_r["lead_bin_decomp"], "15-30") - _bin_iou(base_r["lead_bin_decomp"], "15-30")
        b_3145_d = _bin_iou(tc_r["lead_bin_decomp"], "31-45") - _bin_iou(base_r["lead_bin_decomp"], "31-45")
        short_gain = b_1530_d + b_3145_d
        ok_short = "OK " if short_gain > 0.30 else "FAIL"
        print(f"  [{ok_short}] short-lead gain: bin15-30 Δ={b_1530_d:+.4f}  bin31-45 Δ={b_3145_d:+.4f}  "
              f"sum={short_gain:+.4f}  (thr > +0.30)", flush=True)

        # 4) mu_mean ≈ L + ε (asym weakening hypothesis)
        s_tc = tc_r["mu_sanity"]
        if "mu_off60_minus_L_mean" in s_tc and "target_offset_expected" in s_tc:
            ...
            ok_mu = "OK " if abs(gap) <= 3.0 else "WARN"
            print(f"  [{ok_mu}] mu_off60 − L mean = {s_tc['mu_off60_minus_L_mean']:+.2f}  "
                  f"(expected ≈ ε = {s_tc['target_offset_expected']}, gap = {gap:+.2f}; tol ±3)", flush=True)
        # Also bins 61-90 (no-regression check)
        b6190_base = _bin_iou(base_r["lead_bin_decomp"], "61-90")
        b6190_tc = _bin_iou(tc_r["lead_bin_decomp"], "61-90")
        print(f"  [INFO] bin 61-90 IoU base={b6190_base:.4f} → tc={b6190_tc:.4f}  "
              f"(Δ = {b6190_tc - b6190_base:+.4f})", flush=True)
