"""Phase S7 inference — refresh sample_grid with the asym-weakened ε-target ckpt.

Calls `phase_r_oracle_iou` with two models held side-by-side:
    [A] D=15 2-sided (asym=25)               (original baseline)
    [B] D=15 2-sided tc7p5_asym15ae0         (this experiment)

Both use σ=5.0.  Offsets = {60, 90, 105, 120} (Phase S3 C_old setup).

Outputs:
    outputs_phase_s7_sample_grid.csv
    outputs_phase_s7_oracle.csv
"""
...
def main():
    ...
    ap.add_argument("--baseline_2sided_ckpt", type=str,
                    ...
                    help="Original 2-sided ckpt (target_offset=0, asym=25/5) for comparison row.")
    ap.add_argument("--tc_ckpt", type=str, required=True,
                    help="Path to the new ε-target asym-weakened ckpt (Phase S7 final).")
    ap.add_argument("--tc_label", type=str, default="D=15 2-sided tc7p5_asym15ae0",
                    help="Label used in the sample_grid 'model' column for the experiment row.")
    ap.add_argument("--sigma_2sided", type=float, default=5.0)
    ap.add_argument("--sigma_tc", type=float, default=5.0)
    ...
    ap.add_argument("--out_csv", type=str, default="rice/outputs/diag/phase_s7_oracle.csv")
    args = ap.parse_args()
    ...
    models = (f"{base_label}|{args.baseline_2sided_ckpt};"
              f"{args.tc_label}|{args.tc_ckpt}")
    per_model_sigma = (f"{base_label}={args.sigma_2sided},"
                       f"{args.tc_label}={args.sigma_tc}")
    ...
    cmd = [
        ...
        "--oracle_sigma", str(args.sigma_tc),
    ...
    print("=" * 70, flush=True)
    print("Phase S7 inference (phase_r_oracle_iou)", flush=True)
