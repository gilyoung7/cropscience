"""
Phase I v2 — Variable offset with GBM regressor + extra features.

Same pipeline as phase_i_variable_offset but:
  - Regressor = sklearn.ensemble.GradientBoostingRegressor
                (n_estimators=200, max_depth=3, learning_rate=0.05)
  - Features  = 4 Stage1 score stats + alert_tstar + lat + lon + year

Goal: widen pred_L distribution (std → 15+) so the per-site-year offset
covers a larger range than the v1 linear model (std=11.5, clipped to 105~120).

Inference only; no retraining of Stage 2.
"""
...
def build_seas_meta(seas_list: list[dict], feature_cols: list[str]) -> dict:
...
def group_and_features(samples_now: list[dict], scores: np.ndarray,
                       seas_meta: dict, with_alert: bool, tau: float | None) -> pd.DataFrame:
    ...
    for (site, year), idxs in idx_map.items():
        ...
        for i in idxs:
            ...
            if Li is not None:
                L_val = int(Li)
                break
        lat, lon = seas_meta.get((site, int(year)), (float("nan"), float("nan")))
        alert_t = None
        if with_alert and tau is not None:
            for i in idxs:
        ...
        rows.append({
            ...
            "alert_tstar": alert_t, "lat": lat, "lon": lon,
        })
...
def stage1_pipeline(stage1_ckpt_path: Path, run: int, args):
    ...
    seas_meta = build_seas_meta(train_seas + val_seas + test_seas, feature_cols)
    train_df = group_and_features(train_s, p_train_cal, seas_meta, with_alert=True, tau=tau)
    test_df = group_and_features(test_s, p_test_cal, seas_meta, with_alert=True, tau=tau)
    print(f"[features] train groups={len(train_df)} "
          f"(interval={train_df['L'].notna().sum()}, alerted={train_df['alert_tstar'].notna().sum()})  "
          f"test groups={len(test_df)} "
          f"(interval={test_df['L'].notna().sum()}, alerted={test_df['alert_tstar'].notna().sum()})")
    return train_df, test_df, tau


def fit_gbm_regressor(train_df: pd.DataFrame, args) -> tuple[GradientBoostingRegressor, list[str]]:
    feat_cols = ["score_at_tstar", "score_peak_30", "score_slope_14", "score_auc_30",
                 "alert_tstar", "lat", "lon", "year"]
    ...
    print(f"[regressor:GBM] n_train={len(df)}  R^2={r2:.4f}  MAE={mae:.2f} days  "
          f"(n_estimators={args.n_estimators}, max_depth={args.max_depth}, lr={args.learning_rate})")
    fi = sorted(zip(feat_cols, reg.feature_importances_), key=lambda x: -x[1])
    print("  feature importances (sorted):")
    for c, w in fi:
        print(f"    {c:<16} = {w:.4f}")
    return reg, feat_cols


def stage2_row_map(stage2_ckpt_path: Path, run: int, args, device: torch.device):
...
def main() -> None:
    ...
    print(f"[config] GBM(n_estimators={args.n_estimators}, max_depth={args.max_depth}, "
    ...
    print("\n[2/4] Fit GBM regressor (extra features) on train interval+alerted rows")
    reg, feat_cols = fit_gbm_regressor(train_df, args)

    print("\n[3/4] Apply regressor to test, compute variable offset_i")
    mask_t = test_df[feat_cols].notna().all(axis=1) & test_df["alert_tstar"].notna()
    test_use = test_df[mask_t].copy()
    test_use["pred_L"] = reg.predict(test_use[feat_cols].to_numpy(dtype=float))
    ...
    print(f"  pred_L  mean/std     = {test_use['pred_L'].mean():.1f} / {test_use['pred_L'].std():.1f}")
    ...
    print("\n--- comparison (D=15) ---")
    print(f"  Variable v1 (linear)  : F1=0.6126  IoU80=0.3096  EarlyRecall=0.6825  pred_L_std=11.5")
    print(f"  Variable v2 (GBM)     : F1={f1:.4f}  IoU80={np.mean(ious):.4f}  EarlyRecall={er80:.4f}  "
          f"pred_L_std={test_use['pred_L'].std():.2f}")
    print(f"  fixed offset=60       : F1=0.540   IoU80=0.257   EarlyRecall=0.621")
    print(f"  fixed offset=105      : F1=0.628   IoU80=0.324   EarlyRecall=0.750   ← prev best F1")
    print(f"  fixed offset=120      : F1=0.589   IoU80=0.420   EarlyRecall=0.438   ← best IoU")
