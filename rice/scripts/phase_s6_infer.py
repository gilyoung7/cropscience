"""Phase S6 inference — refresh sample_grid with the ε-target 2-sided ckpt.

Calls `phase_r_oracle_iou` with two models held side-by-side:
    [A] D=15 2-sided (asym=25)            (original target=L)
    [B] D=15 2-sided tc{tag} (target=L+ε) (this experiment)

Both use σ=5.0.  Offsets remain the original {60, 90, 105, 120} (matches
Phase S3 C_old).

Outputs:
    outputs_phase_s6_sample_grid.csv
    outputs_phase_s6_oracle.csv
"""
...
def main():
    ...
    ap.add_argument("--baseline_2sided_ckpt", type=str,
                    ...
                    help="Original 2-sided ckpt (target_offset=0) for the comparison row.")
    ap.add_argument("--tc_ckpt", type=str, required=True,
                    help="Path to the new ε-target 2-sided ckpt (Phase S6 final).")
    ap.add_argument("--tc_label", type=str, default="D=15 2-sided tc7p5",
                    help="Label used in the sample_grid 'model' column for the ε-target model.")
    ...
    ap.add_argument("--out_csv", type=str, default="rice/outputs/diag/phase_s6_oracle.csv")
    args = ap.parse_args()
    ...
    print("=" * 70)
    print("Phase S6 inference (phase_r_oracle_iou)")
    print(" ".join(cmd))
    print("=" * 70)
    env = {**os.environ,
