...
def main():
    ...
    # The bucket assignment depends ONLY on the dispatch CSV's val sample_ids (pure md5 hash),
    # never on the grid -- the grid is used solely for the row-count columns of the manifest.
    # Make it optional so a clean-only smoke cell can run before any dev grid exists.
    gpath = PP.dev_grid(a.pest)
    if gpath.exists():
        grid = pd.read_csv(gpath)
        grid = grid[(grid.variant == "E5d") & (grid.offset.isin(PP.OFFSETS))]
    else:
        print(f"[splits] dev grid absent ({gpath.name}); manifest row-counts will be 0. "
              f"Bucket assignment is unaffected.")
        grid = pd.DataFrame(columns=["year", "split", "sample_id", "offset", "L", "R", "alert_tstar"])
