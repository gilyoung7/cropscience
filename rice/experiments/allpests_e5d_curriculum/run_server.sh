...
echo "[server$SERVER] pre-flight: final-argument parity vs scratch arm"
cd "$CS"
if ! bash "$AP/verify_final_parity.sh" >/dev/null; then
  echo "[server$SERVER] ABORT: final-argument parity FAILED -- fix before training"; exit 3
fi
