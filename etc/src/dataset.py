...
def split_by_sample(samples: list[dict], val_frac=0.1, test_frac=0.1, seed=42):
    rng = np.random.default_rng(seed)
    n = len(samples)
    if n == 0:
        return [], [], []
    ...
    # If fractions are non-zero, try to keep at least 1 val/test when feasible.
    if test_frac > 0 and n_test == 0 and n >= 3:
        n_test = 1
    if val_frac > 0 and n_val == 0 and n - n_test >= 2:
        n_val = 1

    # Final guard to keep at least one train sample.
    if n_test + n_val >= n:
...
def split_seed_search_topk(
    ...
    for seed in seed_candidates:
        train_s, val_s, test_s = split_by_sample(samples, val_frac=val_frac, test_frac=test_frac, seed=seed)
