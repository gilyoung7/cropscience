...
from etc.src.ckpt_schema import build_ckpt_meta
from etc.src.model import HazardTransformer
from etc.src.train_eval import run_epoch_weighted, eval_nll_model, eval_metrics_with_overlap
...
def main(
    ...
    if len(val_s) == 0 and len(train_s) > 0:
        print("[warn] val_s is empty; falling back to train_s for validation metrics.")
        val_s = train_s
